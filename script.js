const RXMLParser = new XMLParser({
...
function getByClass(Table,Name,ObjName){
  for (const Index in Table){
    if (Table[Index][ObjName || '@_class'] && Table[Index][ObjName || '@_class'] == Name){
      return(Table[Index])
    }
  }
}

function getDescendantsOfClass(Table,Name){
  let ReturnTable = []
  for (const Index in Table){
    ReturnTable = ReturnTable.concat(getDescendantsOfClass(Table[Index].Item,Name))
    if (Table[Index]['@_class'] && Table[Index]['@_class'] === Name){
      ReturnTable.push(Table[Index])
    }
  }
  return(ReturnTable)
}

function G(Table,Name){
  if (Array.isArray(Table)) {
    return(getByClass(Table,Name,"@_name"))
  }else{
    return(Table)
  }
}

function solidify(Workspace){
  const SPS = new BABYLON.SolidParticleSystem("SPS", scene)
  const BOX = BABYLON.MeshBuilder.CreateBox("BasePart",{})
  const Parts = getDescendantsOfClass(Workspace.Item,"Part")
  SPS.addShape(BOX,Parts.length)
  BOX.dispose()
  SPS.buildMesh()
  ...
  for (const Index in Parts){
    ...
    const Size = G(Part.Properties.Vector3,"size")
    const CFrame = G(Part.Properties.CoordinateFrame,"CFrame")
    const BrickColor = G(Part.Properties.int,"BrickColor")
    const Color = BrickColor && brickcolor[BrickColor["#text"]] || color3uint8(G(Part.Properties.Color3uint8,"Color3uint8")["#text"])
    const rotY = Math.asin(clamp(CFrame.R02, - 1, 1 ));
    if ( Math.abs(CFrame.R02) < 0.99 ) {
      rotX = Math.atan2( - CFrame.R12, CFrame.R22);
      rotZ = Math.atan2( - CFrame.R01, CFrame.R00);
    } else {
      rotX = Math.atan2( - CFrame.R22, CFrame.R11);
      rotZ = 0;
    }
    Particle.position = new BABYLON.Vector3(CFrame.X,CFrame.Y,-CFrame.Z)
    Particle.scale = new BABYLON.Vector3(Size.X,Size.Y,Size.Z)
    Particle.rotation = new BABYLON.Vector3(rotX,rotY,rotZ)
    Particle.color = new BABYLON.Color4(Color[0]/255,Color[1]/255,Color[2]/255,1)
  }
  SPS.setParticles();
}

fetch('offlimiumtestplace.rbxlx').then(response => response.text()).then(function(data){  
  console.log('Game Loaded')
  const game = RXMLParser.parse(data).roblox.Item
  const workspace = getByClass(game,"Workspace")
  console.log('Game Parsed, now rendering')
  solidify(workspace)
...
const properties = v.getElementsByTagName("Properties")[0]
      ...
      if ( Math.abs( nodeValue(cf.getElementsByTagName("R02")) ) < 0.9 ) {
